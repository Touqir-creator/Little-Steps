...
const respondToBooking = async (bookingId, status) => {
  ...
  return <main>
    ...
    <section className="finder-wrap" id="find-care"><div className="container"><form className="finder" onSubmit={async (event) => { event.preventDefault(); if (search.location.trim() && search.location !== 'Near me') { try { const { lat, lon } = await geocodePlace(`${search.location}, India`); fetchProviders({ lat, lng: lon }) } catch { fetchProviders() } } document.querySelector('#providers').scrollIntoView({ behavior: 'smooth' }) }}><label>Where?<div className="where-row"><input value={search.location} onChange={(event) => setSearch({ ...search, location: event.target.value })} placeholder="Your area or city" /><button type="button" className="locate-btn" onClick={useMyLocation} disabled={locating}>{locating ? 'Locating...' : 'Use my location'}</button></div></label><label>When?<input type="date" min={tomorrow} value={search.date} onChange={(event) => setSearch({ ...search, date: event.target.value })} /></label><label>What kind of care?<select value={search.care} onChange={(event) => setSearch({ ...search, care: event.target.value })}><option value="">Any type of care</option><option value="babysitting">Babysitting</option><option value="daycare">Daycare</option><option value="emergency">Emergency care</option></select></label><button className="search-button">Search care -&gt;</button></form></div></section>
